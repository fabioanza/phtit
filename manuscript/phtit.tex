...
\def\ourAbstract{%
We propose a novel theoretical framework to describe the transport of information in quantum systems. 
This is aimed at modeling and understanding how out of equilibrium open quantum systems move information 
across their quantum state space, using an approach inspired by transport theories. The main goal is to build 
new mathematical tools, together with physical intuition, to improve our understanding of complex (many-body and
out-of-equilibrium) quantum systems. In particular, we are aiming at unraveling the interplay between dynamical 
properties and information-theoretic features. The main rationale here is to have a framework that can imitate, 
and potentially replicate, the decades-long history of empirical success of classical transport theories in modeling 
...
\end{equation}
This is our first and most important result. It is the central tool which regulates 
how quantum information is scrambled around the state space, due to the interaction between a 
...
What happens when the system is closed? Do the information flux $J_t$ and source $\sigma_t$ have a specific form?
Here we answer both these questions in a detailed manner. To ease the presentation of the results, in this subsection we will 
assume that $\mu_t(dV_{FS})=q_t(Z)dV_{FS}$, where $q_t$ is an absolutely continuous probability density. Since the evolution is Hamiltonian, 
we can use Hamilton's equations of motion to derive the evolution equation of $q_t$.
First, the sink/sources terms vanishes, since the evolution is Hamiltonian, thus preserving the local probabilities.
This supports the idea that a $\sigma_t \neq 0$ is related to the loss of information, and possibly to the existence of a 
dissipative dynamics. Second, starting from the definition of the flux in Eq.(\ref{eq:flux}), we can explicitly write the form of the flux $J_t$, 
using Hamilton's equations of motion. Writing $\Gamma_\alpha$ in canonical coordinates $(\vec{p}(\Gamma_\alpha),\vec{\phi}(\Gamma_\alpha))$, 
we have 
\begin{align}
\dot{\Gamma}_\alpha & = \left( \frac{d\vec{p}}{dt}(\Gamma_\alpha), \frac{d\vec{\phi}}{dt}(\Gamma_\alpha)\right) \nonumber \\
& = \left( \frac{1}{\hbar}\frac{\partial E}{\partial \vec{\phi}}, -\frac{1}{\hbar}\frac{\partial E}{\partial \vec{p}}  \right) = v_H(\vec{p}(\Gamma_\alpha),\vec{\phi}(\Gamma_\alpha))
\end{align}
Note how the definition of the flow in the right-hand side, $v_H$, does not depend on the index $\alpha$: all 
$\Gamma_\alpha$ evolve according to the same (Hamiltonian) flow. Inserting this into the definition of the flux 
...
\begin{align}
\frac{d q_t}{d t} &= \sum_\alpha \frac{\partial q_t}{\partial p_\alpha} \frac{dp_\alpha}{dt}+\frac{\partial q_t}{\partial \phi_\alpha} \frac{d\phi_\alpha}{dt} + \frac{\partial q_t}{\partial t} \nonumber\\
& = \left\{ q_t, E\right\} - \nabla \cdot q_t v_H \nonumber \\
& = 0
\end{align}
