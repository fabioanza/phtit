...
% What is a Quantum State?
A kinetic theory of quantum information transport
}
...
However, since here $\mathcal{E}= \vec{h} \cdot \vec{S}$ and, by using the algebra of poisson brackets,
one can prove that $\vec{S}$ are the generators of $SO(3)$, here we recognize the geometric interpretation
in terms of a $3D$ rotation of the generic expectation value around the axis $\vec{h}$, with angle $(t - t_0)$. 
...
\subsection*{Heisenberg environment}

We now look at the case of a single qubit which interact with a dipole-dipole 
interaction with an environment made by a $1D$ chain of qubits evolving with
a Heisenberg Hamiltonian
\begin{equation}
H = \sum_{k=0}^{L-1} J_x \sigma_k^x \sigma_{k+1}^{x} + J_y \sigma_k^y \sigma_{k+1}^{y} + J_z \sigma_k^z \sigma_{k+1}^{z} + \vec{B} \cdot \vec{\sigma}_k~,
\end{equation}
with $\vec{B}$ a homogeneous magnetic field.
