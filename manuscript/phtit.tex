...
\end{equation}
The remaining step is to connect this with the quantities that determine the geometric quantum state of the system: $x_\alpha$ and $\ket{\chi_\alpha}$.
This is easily done by using the original definition of $\ket{\Phi_\alpha}$. First, since $x_\alpha = \braket{\Phi_\alpha}{\Phi_\alpha}$, we can derive 
the evolution equation for $x_\alpha(t)$ by using Eq.(\ref{eq:kinetic}):
\begin{equation}
...
\end{equation}
Second, the evolution for $\ket{\chi_\alpha}$ is inherited from the ones of $\ket{\Phi_\alpha}$ since $\ket{\chi_\alpha} = \frac{\ket{\Phi_\alpha}}{||\ket{\Phi_\alpha}||}$.
Plugging Eq.(\ref{eq:kinetic}) into the definition for $\ket{\chi_\alpha}$ and applying some simple algebraic manipulations we get
\begin{align}
i\hbar \frac{d }{dt}\ket{\chi_\alpha} &= \left( \hat{H}_\alpha- \frac{i \hbar }{2} \frac{d\log x_\alpha}{dt} \mathbb{I}_S\right) \ket{\chi_\alpha} \nonumber \\
&\quad +\sum_{\beta \neq \alpha } \sqrt{\frac{x_\beta}{x_\alpha}} \hat{V}_{\alpha \beta}\ket{\chi_\beta}~.
\end{align}
