...
\begin{equation}
\frac{\partial \mu_t}{\partial t} = \sum_\alpha \dot{x}_\alpha \delta_{\Gamma_\alpha} - x_\alpha \dot{\Gamma}_{\alpha} \delta^{'}_{\Gamma_\alpha} ~.\label{eq:cont}
\end{equation}
...
\end{equation}
This novel equation is our first and most important result. It is the central tool which regulates 
how quantum information is scrambled around the state space, due to the interaction between a 
system and its environment. 

Before we move on to use the equation to study the flow of quantum information in some concrete
...
of these situations we send the interested reader to Ref.\cite{AnzaC-Qidge}.
...
Third, the derivation was performed using the generic measure $\mu_t$, thus holding also for $q_t(Z)$, when it exists.
...
& = \left\{ \frac{1}{\hbar}\frac{\partial E}{\partial \phi_n}, -\frac{1}{\hbar}\frac{\partial E}{\partial p_n}  \right\}_n = v_H(\vec{p},\vec{\phi})
\end{align}
Note how the right-hand side, $v_H$, does not depend on the index $\alpha$ anymore: all 
...
{\bf TO CHECK THIS WITH THE PROPER DEFINITION OF DIVERGENCE}\\
