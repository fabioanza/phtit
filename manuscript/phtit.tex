...
assume that $\mu_t(dV_{FS})=q_t(Z)dV_{FS}$, where $q_t$ is an absolutely continuous probability density. Since the system
is isolated we have $\dot{x}_\alpha = 0$. This can be seen from the definitions $U_S(t) \otimes \mathbb{I}_E  \ket{\psi_{SE}} = U_S(t) \otimes \mathbb{I}_E  \sum_\alpha \sqrt{x_\alpha} \ket{\chi_\alpha}\ket{e_\alpha} = \sum_\alpha \sqrt{x_\alpha} \ket{\chi_\alpha(t)}\ket{e_\alpha}$, where $\ket{\chi_\alpha(t)} = U_S(t) \ket{\chi_\alpha}$. This proves that 
the sink/source term vanishes: the evolution is Hamiltonian, thus preserving the local probabilities and the shape of the geometric quantum state.
...
\end{align}
Note how the flow in the right-hand side, $v_H$, does not depend on the index $\alpha$: all 
...
& = 0
\end{align}
The treatment of isolated quantum systems concludes the section about the general features of this
new framework. We now turn to the case of an open quantum system.
