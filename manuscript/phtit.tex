...
$\left\{ \ket{b_\alpha}\right\}_{\alpha=0}^{d_E}$ a basis for the Hilbert space $\mathcal{H}_E$ 
...
interacts with an environment is given in Figure \ref{fig:gqs_dynamics}. Since overall information 
has to be preserved, $\int_{\PH} q_t(Z) dV_{\mathrm{FS}} = \sum_\alpha x_\alpha(t) =1$,
...
\begin{equation}
\sigma_t(Z) = \sum_{\alpha=0}^{d_E} \dot{x}_\alpha(t) \widetilde{\delta}\left(Z - \Gamma_\alpha(t) \right)~.
\end{equation}
...
\begin{figure}[t!]
\centering
\includegraphics[width=.4\textwidth]{./img/Flux.pdf}
\caption{TBD
	}
\label{fig:flux_term}
\end{figure}
...
in Figures \ref{fig:flux_term} and \ref{fig:sigma_term}. First, $J$ is an information flux, in the proper sense of flux. As one can appreciate 
...
\begin{figure}[t!]
\centering
\includegraphics[width=.4\textwidth]{./img/Source.pdf}
\caption{TBD
	}
...
in Figure \ref{fig:sigma_term}, 

Therefore, this term possesses 
the hallmarks of sinks and sources terms in general transport theories: it is not associated with particles moving
around the state space, and it can not be rewritten as a divergence term.




If the system is closed, upon unitary evolution the distribution remains a Dirac delta, but 
